...
export default function Allorders() {
  ...
  const orderIdToScroll = searchParams.get("orderId"); // orderId من query
  const activeTab = searchParams.get("tab") || "defaultTab"; // التاب الحالي
  ...
  useEffect(() => {
    if (!loading && orderIdToScroll) {
      // scroll للأوردر الجديد بعد تحميل الأوردرز
      const orderElement = document.getElementById(orderIdToScroll);
      orderElement?.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }, [loading, orderIdToScroll]);
    ...
    <>
      {loading ? (
        ...
        <div className="p-4 md:p-6 space-y-6">
          ...
          {/* يمكنك هنا إضافة Tabs إذا أردت استخدام activeTab */}
